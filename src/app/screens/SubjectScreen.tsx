...
export function SubjectScreen({ subject, progress, onBack, onOpenQuiz }: { subject: Subject; progress: QuizProgress[]; onBack: () => void; onOpenQuiz: (quiz: Quiz) => void }) {
  return <Container maxWidth="md" sx={{ py: 5 }}>
    ...
    <Stack spacing={2}>{progress.map(({ quiz, active, completionCount, currentQuestion }) => <Card key={quiz.id}><CardContent>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2}>
        <Box><Typography variant="h6">{quiz.name}</Typography>
          ...
          {completionCount > 0 && <Chip label={`Completed ${completionCount} ${completionCount === 1 ? 'time' : 'times'}`} size="small" variant="outlined" sx={{ mt: 1 }} />}
        </Box>
