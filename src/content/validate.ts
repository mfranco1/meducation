...
export function validateStoredQuestionBank(bank: StoredQuestionBank): ValidationIssue[] {
 ...
 bank.subjects.forEach(subject => { if (!/^s[1-9]\d*$/.test(subject.id)) issues.push({ level: 'error', message: `Subject ${subject.id} does not use a compact ID` }); });
 bank.quizzes.forEach(quiz => { if (!/^q[1-9]\d*$/.test(quiz.id)) issues.push({ level: 'error', message: `Quiz ${quiz.id} does not use a compact ID` }); });
