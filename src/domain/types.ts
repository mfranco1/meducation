...
export interface AttemptRepository { list(): CompletedAttempt[]; completionCount(quizId: string): number; lowestScore(quizId: string): number | undefined; latestScore(quizId: string): RecentScore | undefined; latestActivityAt(quizId: string): string | undefined; getActive(quizId: string): Attempt | undefined; saveActive(attempt: Attempt): void; clearActive(quizId: string): void; saveCompleted(attempt: CompletedAttempt): void }
