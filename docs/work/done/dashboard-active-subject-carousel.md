...
- [x] Add an accessible, smoothly animated active-subject carousel below the dashboard statistics.
...
- `npm test -- --maxWorkers=1 --minWorkers=1` — passed (17 files, 81 tests). The default parallel run timed out in two existing slow tests under machine contention; both pass in the serial verification run.
